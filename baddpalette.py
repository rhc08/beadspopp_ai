...
def extract_palette(img_bytes, k=8, top_n=5, crop_frac=0.6):
    ...
    # --- map each centroid to the nearest bead shade (no exact repeats) ---
    seen_rgb = set()
    suggestions = []

    for idx in top_idxs:
        rgb_centroid = tuple(centers[idx].astype(int))
        bead = _closest_bead(rgb_centroid)
        
        if tuple(bead["rgb"]) not in seen_rgb:
            seen_rgb.add(tuple(bead["rgb"]))
            suggestions.append(bead)
